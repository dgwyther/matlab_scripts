...
% wind. Below I'm using the |sst| dataset to mask out land values, and I'm using <https://www.mathworks.com/matlabcentral/fileexchange/61293-imagescn
% |imagescn|>, which makes |NaN| values transparent.  After plotting the wind stress, I'm plotting a 
% wind vectors, downsampled by |imresize|, which applies antialiasing when downsampling: 
...
hold on
quiver(imresize(lon,0.05),imresize(lat,0.05),imresize(u10,0.05),imresize(v10,0.05),'k')
...
quiver(imresize(lon,0.05),imresize(lat,0.05),imresize(Taux,0.05),imresize(Tauy,0.05),'b')
...
quiver(imresize(lon,0.05),imresize(lat,0.05),imresize(Taux_wrong,0.05),imresize(Tauy_wrong,0.05),'r')
