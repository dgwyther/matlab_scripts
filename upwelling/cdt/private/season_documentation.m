...
T = 4*sin(doy(t,'yearfraction')*2*pi) + (t-min(t))/5e3 + randn(size(t)) + 15; 
...
T2 = [T + (t-min(t))/2e3;
      T - 15*cos(doy(t,'yearfraction')*2*pi) + (t-min(t))/1e2;
      T + ((t-min(t))/2.5e3).^3.1]; 
...
figure
imagescn(lon,lat,max(sst_season,[],3)-min(sst_season,[],3))
cb = colorbar; 
