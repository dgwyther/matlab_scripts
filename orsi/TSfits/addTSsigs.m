function [c,h]=addTSsigs(rp,conts,col)

% function [c,h]=addTSsigs(rp,conts,col)
%
% Inputs: 
%   rp = reference pressure of computed potential density anomaly (sigma-pr)
%
%   conts = specific values of sigma-pr
%
%   col =
...
%
% Example: 
%   after calling [h,t]=addTSgamfits('SO',[27.80; 28; 28.20; 28.40],6);
%   [c,h]=addTSsigs(0,[27.80; 28; 28.20; 28.40],5);
%   [c,h]=addTSsigs(2000,[37; 37.20; 37.30; 37.40],9);

if (nargin <2 | nargin > 3)
    eval(['help addTSsigss']);
    return;
elseif (nargin == 2); 
col=[8];
end;
...
salmin=v(1);salmax=v(2);deltasal=salmax-salmin;
ptmmin=v(3);ptmmax=v(4);deltaptm=ptmmax-ptmmin;

if (nargin == 1);
pdenmin=sw_dens(salmin,ptmmax,rp)-1000.;
pdenmax=sw_dens(salmax,ptmmin,rp)-1000.;
disp(['current axis has a sig-' num2str(rp) ' range of:']);
disp([' min = ' num2str(pdenmin) '; max = ' num2str(pdenmax) ';']);
disp([' try addsigs again entering CONTS within this range!']);
c=[];h=[];
return;
end;

salg=[salmin:deltasal/100:salmax];
ptmg=[ptmmin:deltaptm/100:ptmmax];
[salg,ptmg]=meshgrid(salg,ptmg);
pdeng=sw_dens(salg,ptmg,rp)-1000.;

conts=sort(conts);
conts=conts(:);
if (length(conts)==1);
    disp([' there was a single contour as input! ']);
    conts=[conts; conts];
end;

[c,h] = contour(salg,ptmg,pdeng,conts,'-');
set(h,'LineWidth',0.25,'color',cbar{col});
clabel(c,h,'label',200,'fontsize',10,'color',cbar{col});
